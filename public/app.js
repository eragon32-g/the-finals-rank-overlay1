...
function rankPartsFromBadgeFile(value) {
  const clean = String(value || "").toLowerCase().replace(".png", "").trim();
  const match = clean.match(/^(bronze|silver|gold|platinum|diamond)-([1-4])$/);
  if (match) {
    return { league: match[1].charAt(0).toUpperCase() + match[1].slice(1), division: match[2] };
  }
  if (clean === "ruby") return { league: "Ruby", division: "" };
  return { league: "", division: "" };
}

function manualRankDisplayFromParams() {
  const fileParts = rankPartsFromBadgeFile(params.get("badgeFile") || "");
  const leagueBase = params.get("league") || fileParts.league || "Unranked";
  const division = params.get("division") || fileParts.division || "";
  return normalizeRankDisplay(`${leagueBase}${division ? " " + division : ""}`.trim());
}
...
async function loadManual() {
  ...
  const fileParts = rankPartsFromBadgeFile(badgeFile);
  const leagueBase = params.get("league") || fileParts.league || "Unranked";
  const division = params.get("division") || fileParts.division || "";
  ...
  try { forceManualRankDisplay(); } catch {}
}
...
/* RankTag BETA 0.0.3 Plus style finalizer */
...
/* RankTag BETA 0.0.3 - single source image-base Plus engine */
...
/* RankTag BETA 0.0.3 render marker */
document.documentElement.setAttribute("data-ranktag-version", "0.0.3");
...
/* RankTag BETA 0.0.3 - Premium backdrop engine: Cyber Red Elite keeps the stable CSS layout and adds a premium skin image */
  ...
  function ensureStyle() {
    ...
    style.textContent = `
      ...
      #badge.badge.rt78-cyber-red-elite {
        ...
        background: transparent url('/assets/premium/cyber-red-elite-bg-v96.png?v=003') center / 100% 100% no-repeat !important;
...
`;
    style.textContent += `
/* RankTag BETA 0.0.3 - locked premium coordinates from Layout Editor */
#badge.rt78-cyber-red-elite .rank-mark,
...
/* RankTag BETA 0.0.3 - Premium backdrop engine: full 470x160 background adapted to stable layout */
...
/* RankTag BETA 0.0.3 - runtime premium layout from URL param */
...
/* RankTag BETA 0.0.3 - AUTHORITATIVE premium layout sync */
...
/* RankTag BETA 0.0.3 - DETACHED PREMIUM RENDERER
...
(function rankTagDetachedPremiumRendererV92(){
  ...
  function getCurrentData() {
    ...
    const badgeSrc = badgeImage?.getAttribute("src") || rankIcon?.querySelector("img")?.getAttribute("src") || "/assets/badges/platinum.svg?v=003";
  ...
  function renderDetached() {
    ...
    root.innerHTML = `
      <img class="rt92-layer-bg" src="${esc(layout.background)}?v=003" alt="" />
...
/* RankTag BETA 0.0.3 - MULTI PREMIUM DETACHED RENDERER */
(function rankTagMultiPremiumDetachedRendererV97(){
  ...
  function getCurrentData() {
    ...
    const brandValue = (brandMarqueeText?.textContent || "ERDRAGON32 • Join the Discord").trim();
    const badgeSrc = badgeImage?.getAttribute("src") || "/assets/badges/platinum.svg?v=003";
    return { rankValue, scoreValue, playerValue, brandValue, badgeSrc };
  }
  ...
  function renderDetached() {
    ...
    root.innerHTML = `
      <img class="rt97-layer-bg" src="${esc(layout.background)}?v=003" alt="" />
...
/* RankTag BETA 0.0.3 - hard fix manual rank display */
function forceManualRankDisplay() {
  if (!isManualMode()) return;
  const rankValue = manualRankDisplayFromParams().toUpperCase();
  const scoreLabel = (params.get("scoreLabel") || "ELO").toUpperCase();
  const scoreValue = compactNumber(params.get("rankScore") || params.get("score") || params.get("points") || "0");
  const playerValue = getPlayerFromUrl();

  if (rankText) rankText.textContent = rankValue;
  if (scoreText) scoreText.textContent = `${scoreLabel}: ${scoreValue}`;
  if (nameText) nameText.textContent = playerValue;

  document.querySelectorAll(".rt103-rank,.rt97-rank,.rt92-rank,.rank-text").forEach((el) => {
    el.textContent = rankValue;
  });
  document.querySelectorAll(".rt103-score,.rt97-score,.rt92-score,.score-text").forEach((el) => {
    el.textContent = `${scoreLabel}: ${scoreValue}`;
  });
  document.querySelectorAll(".rt103-player,.rt97-player,.rt92-player,.name-text").forEach((el) => {
    el.textContent = playerValue;
  });
}

if (isManualMode()) {
  window.addEventListener("load", () => {
    setTimeout(forceManualRankDisplay, 60);
    setTimeout(forceManualRankDisplay, 250);
    setTimeout(forceManualRankDisplay, 900);
    setTimeout(forceManualRankDisplay, 1600);
  });
}

/* RankTag BETA 0.0.3 - FINAL AUTHORITATIVE PREMIUM RENDERER */
(function rankTagV103FinalPremiumRenderer(){
  ...
  function data() {
    if (typeof isManualMode === "function" && isManualMode()) {
      const manual = currentManualData ? currentManualData() : null;
      return {
        rank: manual?.rank || (typeof manualRankDisplayFromParams === "function" ? manualRankDisplayFromParams().toUpperCase() : "PLATINUM 1"),
        score: manual?.score || "ELO: 0",
        player: manual?.player || (typeof getPlayerFromUrl === "function" ? getPlayerFromUrl() : "NOMEPLAYER#1234"),
        brand: (brandMarqueeText?.textContent || "ERDRAGON32 • Join the Discord").trim(),
        badge: badgeImage?.getAttribute("src") || "/assets/badges/platinum.svg?v=003"
      };
    }
    return {
  ...
  function render() {
    ...
    root.innerHTML = `
      <img class="rt103-bg" src="${esc(layout.background)}?v=003" alt="" />
...
/* RankTag BETA 0.0.3 - premium manual rank sync final override */
(function rankTagPremiumManualRankSync003(){
  function isManual() {
    try {
  ...
  function partsFromBadgeFile(value) {
    const clean = String(value || "").toLowerCase().replace(".png", "").trim();
    const match = clean.match(/^(bronze|silver|gold|platinum|diamond)-([1-4])$/);
    if (match) {
      return {
        league: match[1].charAt(0).toUpperCase() + match[1].slice(1),
        division: match[2]
      };
    }
    if (clean === "ruby") return { league: "Ruby", division: "" };
  ...
  function prettyRank(value) {
    const raw = String(value || "Unranked").trim();
  ...
  function compactNumber(value) {
    ...
    if (!raw) return "0";
    try { return new Intl.NumberFormat("it-IT").format(Number(raw)); }
    catch { return raw; }
  }

  function currentManualData() {
    const params = new URLSearchParams(window.location.search);
    const fileParts = partsFromBadgeFile(params.get("badgeFile") || "");
    const leagueBase = params.get("league") || fileParts.league || "Unranked";
    const division = params.get("division") || fileParts.division || "";
    const rankValue = prettyRank(`${leagueBase}${division ? " " + division : ""}`.trim());
    const scoreLabel = (params.get("scoreLabel") || "ELO").toUpperCase();
    const scoreValue = compactNumber(params.get("rankScore") || params.get("score") || params.get("points") || "0");
    const playerValue = params.get("player") || "NomePlayer#1234";
    return {
      rank: rankValue,
      score: `${scoreLabel}: ${scoreValue}`,
      player: playerValue
    };
  }

  function applyPremiumManualText() {
    if (!isManual()) return;
    const d = currentManualData();

    // Base/default DOM
    document.querySelectorAll("#rankText, .rank-text").forEach((el) => { el.textContent = d.rank; });
    document.querySelectorAll("#scoreText, .score-text").forEach((el) => { el.textContent = d.score; });
    document.querySelectorAll("#nameText, .name-text").forEach((el) => { el.textContent = d.player; });

    // Premium detached renderers used across previous builds
    document.querySelectorAll(".rt103-rank,.rt97-rank,.rt92-rank").forEach((el) => { el.textContent = d.rank; });
    document.querySelectorAll(".rt103-score,.rt97-score,.rt92-score").forEach((el) => { el.textContent = d.score; });
    document.querySelectorAll(".rt103-player,.rt97-player,.rt92-player").forEach((el) => { el.textContent = d.player; });
  }

  function start() {
    if (!isManual()) return;
    applyPremiumManualText();
    [50, 120, 250, 500, 900, 1400, 2200, 3200].forEach((ms) => {
      setTimeout(applyPremiumManualText, ms);
    });

    // Watch premium renderer re-renders and immediately rewrite text if needed.
    const root = document.getElementById("badge") || document.body;
    try {
      const observer = new MutationObserver(() => applyPremiumManualText());
      observer.observe(root, { childList: true, subtree: true, characterData: true });
      setTimeout(() => observer.disconnect(), 10000);
    } catch {}

    // Keep a light interval only in the first seconds of iframe load.
    const interval = setInterval(applyPremiumManualText, 350);
