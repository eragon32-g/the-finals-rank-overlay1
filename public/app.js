...
/* RankTag BETA 0.1.5 - base overlay layout renderer */
...
/* RankTag BETA 0.1.5 - custom image elements renderer */
...
/* RankTag BETA 0.1.5 Plus style finalizer */
...
/* RankTag BETA 0.1.5 - single source image-base Plus engine */
...
/* RankTag BETA 0.1.5 render marker */
document.documentElement.setAttribute("data-ranktag-version", "0.1.5");
...
/* RankTag BETA 0.1.5 - Premium backdrop engine: Cyber Red Elite keeps the stable CSS layout and adds a premium skin image */
  ...
  function ensureStyle() {
    ...
    style.textContent = `
      ...
      #badge.badge.rt78-cyber-red-elite {
        ...
        background: transparent url('/assets/premium/cyber-red-elite-bg-v96.png?v=015') center / 100% 100% no-repeat !important;
...
`;
    style.textContent += `
/* RankTag BETA 0.1.5 - locked premium coordinates from Layout Editor */
#badge.rt78-cyber-red-elite .rank-mark,
...
/* RankTag BETA 0.1.5 - Premium backdrop engine: full 470x160 background adapted to stable layout */
...
/* RankTag BETA 0.1.5 - runtime premium layout from URL param */
...
/* RankTag BETA 0.1.5 - AUTHORITATIVE premium layout sync */
...
/* RankTag BETA 0.1.5 - DETACHED PREMIUM RENDERER
...
(function rankTagDetachedPremiumRendererV92(){
  ...
  function getCurrentData() {
    ...
    const badgeSrc = badgeImage?.getAttribute("src") || rankIcon?.querySelector("img")?.getAttribute("src") || "/assets/badges/platinum.svg?v=015";
  ...
  function renderDetached() {
    ...
    root.innerHTML = `
      <img class="rt92-layer-bg" src="${esc(layout.background)}?v=015" alt="" />
...
/* RankTag BETA 0.1.5 - MULTI PREMIUM DETACHED RENDERER */
(function rankTagMultiPremiumDetachedRendererV97(){
  ...
  function getCurrentData() {
    ...
    const brandValue = (brandMarqueeText?.textContent || "ERDRAGON32 • Join the Discord").trim();
    const badgeSrc = badgeImage?.getAttribute("src") || "/assets/badges/platinum.svg?v=015";
    return { rankValue, scoreValue, playerValue, brandValue, badgeSrc };
  }
  ...
  function renderDetached() {
    ...
    root.innerHTML = `
      <img class="rt97-layer-bg" src="${esc(layout.background)}?v=015" alt="" />
...
/* RankTag BETA 0.1.5 - FINAL AUTHORITATIVE PREMIUM RENDERER */
(function rankTagV103FinalPremiumRenderer(){
  ...
  function data() {
    return {
      ...
      badge: badgeImage?.getAttribute("src") || "/assets/badges/platinum.svg?v=015"
    };
  }
  ...
  function render() {
    ...
    root.innerHTML = `
      <img class="rt103-bg" src="${esc(layout.background)}?v=015" alt="" />
...
/* RankTag BETA 0.1.5 - custom elements load fallback */
...
/* RankTag BETA 0.1.5 - authoritative builder layout renderer */
function renderBaseBuilderLayoutAuthoritative() {
  ...
  const make = (key, cfg, content, className) => {
    ...
    if (key === "badge") {
      node.classList.toggle("final-badge-glow", paramEnabled("baseBadgeGlow", true));
      node.classList.toggle("final-badge-ring", paramEnabled("baseBadgeRing", true));
    }
